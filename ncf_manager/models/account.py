...
class AccountJournal(models.Model):
    ...
    sequence_id = fields.Many2one('ir.sequence',
        help='Secuencia para numero de comprobantes fiscales',
        check_company=True,
        readonly=True, copy=False)
    
    ncf_control = fields.Boolean("Control de NCF")
    prefix = fields.Char(related="sequence_id.prefix")
    date_range_ids = fields.One2many('ir.sequence.date_range','journal_id')
    ...
    def onchange_ncf_control(self):
        ...
        if self.ncf_control:
            sq=self.env['ir.sequence'].search([('id','=',self.env.ref('ncf_manager.sequences_credit_fiscal').id)])
            if not self.sequence_id:
                self.sequence_id=sq.id
            sq.ncf_control=self.ncf_control
        else:
             self.sequence_id.ncf_control=False

    def create_ncf_sequence(self):
        # self.sequence_id=self.env.ref('ncf_manager.sequences_credit_fiscal').id
        if self.ncf_control and len(self.sequence_id.date_range_ids) <= 1:
            ...
            for sale_fiscal_type in selection:
                self.sequence_id.date_range_ids[0].copy({'sale_fiscal_type': sale_fiscal_type[0],'journal_id':self.id})
