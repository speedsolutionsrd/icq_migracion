...
class ResCompany(models.Model):
    ...
    payment_tax_on_606 = fields.Boolean("Reportar retenciones del 606 en la"
                                        " fecha del pago")
...
class DgiiPurchaseReport(models.Model):
    ...
    company_id = fields.Many2one(
        ...
        self.env['res.company']._company_default_get('dgii.purchase.report'))
    name = fields.Char()
    year = fields.Integer(
        u"Año", size=4, default=lambda s: int(time.strftime("%Y")))
    month = fields.Integer(
        "Mes", size=2, default=lambda s: int(time.strftime("%m")))
    ...
    TOTAL_MONTO_FACTURADO = fields.Float("Total Facturad")
    ...
    def create_report_lines(self, invoices, tax_account):
        ...
        for inv in invoices:
            ...
            move_lines = [
             move_line for move_line in inv.move_id.line_ids if move_line.
             account_id.id in account_tax_ids]
            for move_line in move_lines:
                amount = (move_line.debit if move_line.debit > 0
                ...
                if tax_account[move_line.account_id.id] == "itbis":
                 ...
                 move_line.account_id.id] == "ritbis" and inv.state == 'paid':
                    if int(max
                           (inv.payment_move_line_ids).date.split("-")[1]
                           ) == self.month:
                 ...
                 move_line.account_id.id] == "cost" and move_line.tax_line_id:
            ...
            if not inv.partner_id.vat:
                raise exceptions.UserError(u"El número de RNC/Cédula del"
                                           ...
                                           "para el NCF {}".format(
                                            inv.partner_id.name, inv.number))
    ...
    def generate_txt(self):
        ...
        if not (company_fiscal_identificacion or not
                is_identification(company_fiscal_identificacion)):
    ...
    def create_report(self):
        ...
        if self.env.user.company_id.payment_tax_on_606:
            # todo fix 606 when have to place retention payment day
            paid_in_period_with_retention_invoices = self.env[
