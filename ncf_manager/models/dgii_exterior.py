...
class DgiiExteriorReport(models.Model):
    ...
    report_lines = fields.One2many("dgii.exterior.report.line",
                                   "exterior_report_id")
    ...
    def create_report_lines(self, invoices):
        ...
        TOTAL_MONTO_FACTURADO = (sum([line[2]["MONTO_FACTURADO"] for line in
                                     lines]))
    ...
    def create_report(self):
        ...
        exterior_journal_ids = (
            [rec.id for rec in self.env["account.journal"].search([
             ('purchase_type', '=', 'exterior')])
             ])
