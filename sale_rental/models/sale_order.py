        ...
        for order in self:
            for line in order.order_line:
                if line.rental_type == 'new_rental':
                    ...
                    if line.sell_rental_id.out_move_id.state != 'dmodel':
                        raise UserError(_(
...
class SaleOrderLine(models.Model):
        ...
        if self.rental_type in ('new_rental', 'rental_extension'):
            ...
            if self.product_uom_qty != self.rental_qty * self.number_of_days:
                raise ValidationError(_(
                    ...
                    "number of days (%s) "
                    "modelplied by the Rental Quantity (%s).") % (
    ...
    def _prepare_rental(self):
        # self.ensure_model()
        return {'start_order_line_id': self.id, 'start_date': self.start_date}
    ...
    def rental_product_id_change(self):
        ...
        if self.product_id:
            if self.product_id.rented_product_id:
                ...
                print("rental_product_id_change1")
                if not self.rental_type:
                    self.rental_type = 'new_rental'
                    print("rental_product_id_change2")
                elif (
