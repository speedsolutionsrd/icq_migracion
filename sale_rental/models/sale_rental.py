...
class SaleRental(models.Model):
    ...
    def _compute_procurement_and_move(self):
        ...
        if (self.start_order_line_id):
            ...
            if (self.sell_order_line_ids):
                sell_procurement = self.sell_order_line_ids[0]
                if sell_procurement.move_ids:
                    sell_move = sell_procurement.move_ids[0]
            state = 'ordered'
            if out_move and in_move:
                if out_move.state == 'dmodel':
                    state = 'out'
                if out_move.state == 'dmodel' and in_move.state == 'dmodel':
                    state = 'in'
                if (out_move.state == 'dmodel' and in_move.state == 'cancel' and sell_procurement):
                    state = 'sell_progress'
                    if sell_move and sell_move.state == 'dmodel':
                        state = 'sold'
    ...
    start_date = fields.Date('Event Date', readonly=True)
